...
fn parse_complex(s: &str) -> Option<Complex<f64>> {
    match parse_pair(s, ',') {
        Some((r, i)) => Some(Complex {re: r, im: i}),
        None => None
    }
}
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<u32>("10,20xy" , ',' ), None);
...
fn test_parse_complex() {
    assert_eq!(parse_complex("0.25,0.0625"), Some(Complex {re: 0.25, im: 0.0625}));
...
fn main() {
    ...
    if args.len() != 5 {
        writeln!(
            std::io::stderr(),
            "Usage: mandelbrot FILE PIXELS UPPERLEFT LOWERRITH"
        ).unwrap();
        writeln!(
            std::io::stderr(),
    ...
    writeln!(
        std::io::stderr(),
        "Working on {} w/ {} threads", &args[2], threads
    ).unwrap();
